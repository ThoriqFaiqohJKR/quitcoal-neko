<div class="py-12 mx-auto max-w-5xl flex flex-col min-h-screen">
    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            <h1 class="text-2xl font-semibold">Ngopini</h1>

            <a href="{{ route('cms.ngopini.insert', ['locale' => app()->getLocale()]) }}"
    ...
    <div class="border mt-4">
        ...
        <div class="grid grid-cols-12 bg-gray-100 border-b text-sm font-semibold">
            <div class="col-span-5 px-4 py-3">Judul</div>
            <div class="col-span-5 px-4 py-3">Deskripsi</div>
            <div class="col-span-2 px-4 py-3 text-right">Aksi</div>
        </div>

        @forelse ($ngopinis as $item)
                <div
                    class="grid grid-cols-12 border-b text-sm items-start hover:bg-gray-50 {{ $loop->even ? 'bg-gray-100' : 'bg-white' }}">

                    <a href="{{ route('cms.ngopini.edit', [
                'locale' => app()->getLocale(),
                'id' => $item->id
            ]) }}" class="col-span-10 grid grid-cols-10 hover:bg-gray-50">

                        <div class="col-span-5 px-4 py-3 font-medium text-gray-900">
                            {{ $item->judul_id ?? '-' }}
                        </div>

                        <div class="prose col-span-5 px-4 py-3 text-gray-700 max-w-none">
                            {!! $item->deskripsi_id ?? '-' !!}
                        </div>

                    </a>

                    <div class="col-span-2 px-4 py-3 flex justify-end" x-data="{ open: false }" @click.stop>
                        ...
                        <div x-show="open" x-transition.opacity
                            ...
                            <div @click.outside="open = false" class="bg-white border w-full max-w-sm p-5 text-left">
